...
function searchList(searchInList, searchFor) {

    function searchCallback(string) {
        return string.toLowerCase().includes(searchFor.toLowerCase());
    }

    return searchInList.filter(searchCallback);
}

function searchObjList(searchInList, searchFor) {

    function searchCallback(obj) {
        const testSearchFor = searchFor.toLowerCase();
        const testName = obj.name.toLowerCase();
        const testDesc = obj.description.toLowerCase();
        if (testName.includes(testSearchFor) || testDesc.includes(testSearchFor) || obj.tags.find((tag) => tag.toLowerCase().includes(testSearchFor))) {
            return true;
        }
        return false;
    }
...
const filteredList = searchList(simpleList, "bananas");
console.log(filteredList);

const filteredObjList = searchObjList(hikes, "al");
